...
public class HTTPServer implements Runnable
{
	...
	private List<HTTPServerThread> threadList = new ArrayList<>();

	public List<HTTPServerThread> getThreadList() {
		return threadList;
	}
	...
	public void run()
	{
		...
		while (httpServerThread == thisThread) {
			...
			try {
				Logger.d("accept ...");
				sock = accept();
				if (sock != null && commonSocket == null){
					//远程端的Socket地址 ， 播放器端的
					commonSocket = sock;
					Logger.d("sock = " + sock.getRemoteSocketAddress());
				}
			}
			catch (Exception e){
				Logger.d("accept ..." + e);
